...
## Loads all the scenes from the files so they don't need to be added manually
func load_all_scenes(path: String = "res://") -> void:
	...
	while file_name != "":
		if directory.current_is_dir():
			await load_all_scenes(directory.get_current_dir() + "/" + file_name)
		elif file_name.ends_with(".tscn"):
			if scenes.has(file_name.left(-5)):
				printerr(file_name.left(-5) + " scene was overriden with new path as there are duplicates")
			scenes[file_name.left(-5)] = directory.get_current_dir() + "/" + file_name
...
## Changes the entire scene.
func load_scene(scene_id_or_path: String, duration: float = -1.0, force_reload: bool = false) -> void:
...
func get_scene_path(scene_id_or_path: String) -> String:
	...
	push_error("Unknown scene key or path: '%s'. Available keys: %s" % [scene_id_or_path, ", ".join(get_scene_keys())])
	return ""
...
func _play_loading_overlay_enter_transition() -> void:
	...
	if not _active_loading_overlay.has_method(loading_enter_transition_method):
		push_error("Loading overlay root needs a '%s' function." % String(loading_enter_transition_method))
		return
...
func _play_loading_overlay_exit_transition() -> void:
	...
	if not _active_loading_overlay.has_method(loading_exit_transition_method):
		push_error("Loading overlay root needs a '%s' function." % String(loading_exit_transition_method))
		return
...
func _prepare_scene_music_for_selected_scene() -> void:
	...
	if not scene_music_manager.has_method(scene_music_manager_prepare_method):
		push_error("SceneMusicManager needs a '%s' function." % String(scene_music_manager_prepare_method))
		return
...
func _play_prepared_scene_music() -> void:
	...
	if not scene_music_manager.has_method(scene_music_manager_play_prepared_method):
		push_error("SceneMusicManager needs a '%s' function." % String(scene_music_manager_play_prepared_method))
		return
